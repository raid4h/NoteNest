...
class CalendarScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.main_layout = BoxLayout(orientation="vertical", padding=16, spacing=10)

        # Header
        header = BoxLayout(size_hint_y=None, height=50, spacing=10)
        self.prev_btn = Button(
            text="<", size_hint_x=None, width=40,
            background_normal="", background_color=(0.24, 0.19, 0.15, 1),
            color=(0.96, 0.93, 0.86, 1), font_size=18
        )
        self.prev_btn.bind(on_press=self.prev_month)

        self.month_label = Label(
            text="", font_size=18,
            bold=True, color=(0.02, 0.01, 0.01, 1)
        )
        ...
        for day in ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]:
            days_header.add_widget(Label(
                text=day, font_size=12,
                color=(0.43, 0.41, 0.38, 1), bold=True
            ))

        # Calendar grid
        self.calendar_grid = GridLayout(
            cols=7, size_hint_y=None, spacing=4
        )
        ...
        self.task_label = Label(
            ...
            font_size=14, color=(0.43, 0.41, 0.38, 1),
            ...
            for day in week:
                if day == 0:
                    ...
                    btn = Button(
                        ...
                        background_normal="",
                        background_color=(
                            (0.24, 0.19, 0.15, 1) if is_today
                            else (0.84, 0.82, 0.78, 1) if has_tasks
                            else (0.91, 0.88, 0.86, 1)
                        ),
                        color=(
                            (0.96, 0.93, 0.86, 1) if is_today
                            else (0.02, 0.01, 0.01, 1)
                        ),
                        font_size=13,
                        size_hint_y=None,
                        height=36
    ...
    def select_date(self, date_str):
        ...
        for task in tasks:
            row = BoxLayout(
                ...
                padding=[12, 6, 12, 6], spacing=10
            )
            with row.canvas.before:
                Color(*PRIORITY_COLORS.get(task["priority"], (0.95, 0.93, 0.90, 1)))
                self.rect = RoundedRectangle(pos=row.pos, size=row.size, radius=[10])
            row.bind(pos=lambda w, v, r=row: setattr(self.rect, 'pos', v))
            row.bind(size=lambda w, v, r=row: setattr(self.rect, 'size', v))
